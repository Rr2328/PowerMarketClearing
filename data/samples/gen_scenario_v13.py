...
- 期望价格曲线（MCP）：夜谷 330 → 早爬坡 350/370/390 → 早峰 410 → 午间 350 → 晚峰 410 → 夜 330（2026-09-13 方案 A 现实化校准，贴近省级现货均价 290–360）
...
def main():
    ...
    consumers = [
        # (用户名称, 负荷编号, 第1段报价, 基准量)
        ("一号工业园区", "L1", 700.000, round(mean_load * 0.34, 1)),
        ("二号商业综合体", "L2", 600.000, round(mean_load * 0.40, 1)),
        ("三号居民小区", "L3", 500.000, round(mean_load - round(mean_load * 0.34, 1) - round(mean_load * 0.40, 1), 1)),
    ]
    ...
    gens = [
        ("金陵电厂", "#1机组", [(270.000, 150.0, None, None),   # 基荷
                                (300.000, 90.0, None, None),    # 基荷
                                (350.000, None, 590.0, 90.0)]),  # 调峰：cum(460+130)
        ("金陵电厂", "#2机组", [(290.000, 120.0, None, None),   # 基荷
                                (330.000, None, 460.0, 130.0)]),  # 调峰：cum(460)
        ("龙潭电厂", "#1机组", [(320.000, 100.0, None, None),   # 基荷
                                (390.000, None, 790.0, 80.0)]),   # 调峰：cum(680+110)
        ("龙潭电厂", "#2机组", [(370.000, None, 680.0, 110.0)]),  # 调峰：cum(590+90)
        ("龙潭电厂", "#3机组", [(410.000, None, 870.0, 120.0)]),  # 调峰：cum(790+80)
    ]
