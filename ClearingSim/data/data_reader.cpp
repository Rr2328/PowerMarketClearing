...
{rows.clear();
    ...
    const QStringList header =splitCsvLine(in.readLine().trimmed());
...
{rows.clear();
    ...
    header =splitCsvLine(in.readLine().trimmed());
...
{if (header.isEmpty() ||header[0] !=QStringLiteral("period"))
    ...
    if ((header.size() -3) %2 !=0 ||pairCount <1 ||pairCount >10)
    {addError(errors,cfg.prefix,"长表表头段列不成对：第N段出力/第N段报价必须成对出现，且最多 10 段");
        return -1;
    }
...
{out.clear();
    ...
    {const QStringList &c =row.columns;
        ...
        {addError(errors,prefix,QString("第 %1 行申报段不能超过 10").arg(row.lineNumber));
            rowValid =false;
        }
...
{data.clear();
    ...
    {// ---- 窄表兼容（含/不含「机组类型」列，类型列忽略不校验） ----
        ...
        {addError(errors,prefix,"CSV 表头不匹配：应为 V1.3 长表"
...
{data.clear();
    ...
    {appendErrors("generator_bids.csv",fileErrors,errors);
        allOk =false;
    }
    ...
    {appendErrors("consumer_bids.csv",fileErrors,errors);
        allOk =false;
    }
    ...
    {appendErrors("load_curve.csv",fileErrors,errors);
        allOk =false;
    }
...
{errors.clear();
    ...
    {QString line =QStringLiteral("逐时段平衡：%1/%2 个时段申报与负荷偏差超 1 MW").arg(deviatingCount).arg(periodsChecked);
        if (maxShortMW >0.0)
            line +=QStringLiteral("；最大缺口 时段%1%2 申报较负荷少 %3 MW").arg(maxShortT).arg(timeTag(maxShortT)).arg(maxShortMW,0,'f',1);
        if (maxSurplusMW >0.0)
            line +=QStringLiteral("；最大过剩 时段%1%2 申报较负荷多 %3 MW").arg(maxSurplusT).arg(timeTag(maxSurplusT)).arg(maxSurplusMW,0,'f',1);
        hints <<line;
    }
...
{data.clear();
    ...
    if (header !=expectedHeader)
    {addError(errors,QStringLiteral("机组技术参数"),"表头不符，期望 name,id,pMin,pMax,rampUp,rampDown,minUpTime,"
