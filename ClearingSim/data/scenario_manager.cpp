...
{if (period == 24)
    ...
    return QString("%1:00").arg(period,2,10,QChar('0'));
}

// 负荷时段整理
bool buildLoadMap(const QVector<LoadPoint> &data,int expectedCount,QMap<int, LoadPoint> &loadMap,QStringList &errors)
{loadMap.clear();
    if (data.size() != expectedCount)
    {errors.append(QString("负荷数据应包含 %1 个时段，实际为 %2 个").arg(expectedCount).arg(data.size()));
        return false;
    }
    for (const LoadPoint &item : data)
    {if (item.period < 1 || item.period > expectedCount)
        {errors.append(QString("负荷时段 %1 超出 1~%2 范围").arg(item.period).arg(expectedCount));
            continue;
        }
        if (loadMap.contains(item.period))
        {errors.append(QString("负荷时段 %1 重复").arg(item.period));
            continue;
        }
        loadMap.insert(item.period,item);
    }
    for (int period = 1; period <= expectedCount; ++period)
    {if (!loadMap.contains(period))
        {errors.append(QString("负荷数据缺少时段 %1").arg(period));
        }
    }
...
{load24.clear();
    ...
    if (!buildLoadMap(load96,96,loadMap,errors))
    {return false;
    }
    ...
    {double total = 0.0;
        ...
        {total += loadMap.value(firstPeriod + offset).load;
        }
        LoadPoint item;
        item.period = hour;
...
=======
// 构建逐时段场景
bool ScenarioManager::buildPeriodScenarios(const MarketData &data,int periodCount,QVector<PeriodScenario> &scenarios,QStringList &errors)
{scenarios.clear();
    errors.clear();
    QVector<LoadPoint> loadData;
    if (periodCount == 96)
    {loadData = data.loadCurve;
    }
    ...
    {if (data.loadCurve.size() == 24)
        ...
        {QStringList tempErrors;
            if (!aggregateLoadTo24(data.loadCurve,loadData,tempErrors))
    ...
    {errors.append("时段数量只能选择 24 或 96");
        return false;
    }
    QMap<int, LoadPoint> loadMap;
    if (!buildLoadMap(loadData,periodCount,loadMap,errors))
    {return false;
    }
