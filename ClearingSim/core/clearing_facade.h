...
//   只做"构造入参 → 调出清内核 → 聚合结果"三件事。
//   分段撮合走 engine/clearing_engine.cpp 的 ClearMarket（高低匹配）；
//   二次曲线走 core/quadratic_clearing.cpp；SCUC 走 uc/uc_solver.cpp（HiGHS）。
//   结算在 #94 已统一收口至 facade 与各出清内核，引擎不再单独持有 settle()。
//   新能源以 0 价供给段参与撮合（价格接受者，优先中标），三模式共用。
//   V1.3（#67/#88/#94）：申报数据带 period 维度，引擎恒跑 96 期、逐时段取
